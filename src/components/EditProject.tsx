...
const EditProject:React.FC<Props> = ({projectData}) => {
...
setError("")
    ...
    try{
        if(deleteFile){
            ...
            const desertRef = ref(storage, `projects/${id}/image`);
            const downloadURL =   await getDownloadURL(desertRef)

            deleteObject(desertRef).then(() => {
                      updateDoc(doc(db, "projects", `${router.query.id}`), {
  ...
  throw new Error('an error occured')
         ...
         const projectRef = doc(db, "projects", `${router.query.id}`)
